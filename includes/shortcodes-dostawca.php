...
/* ==========================================================
   🔟 SHORTCODY OFERTY (pojedyncza oferta / loop)
   ========================================================== */

if ( ! function_exists( 'bm_get_current_offer_post_id' ) ) {
    function bm_get_current_offer_post_id() {
        $post_id = get_queried_object_id();
        if ( $post_id && get_post_type( $post_id ) === 'dostawca_oferta' ) {
            return (int) $post_id;
        }
        global $post;
        if ( $post instanceof WP_Post && $post->post_type === 'dostawca_oferta' ) {
            return (int) $post->ID;
        }
        return 0;
    }
...
if ( ! function_exists( 'bm_get_offer_term_names' ) ) {
    function bm_get_offer_term_names( $post_id, $taxonomy, $sep = ', ' ) {
        $terms = wp_get_post_terms( (int) $post_id, $taxonomy, [ 'fields' => 'names' ] );
        if ( is_wp_error( $terms ) || empty( $terms ) ) return '';
        return implode( $sep, $terms );
    }
}

add_shortcode( 'bm_oferta_tytul', function() {
    $offer_id = bm_get_current_offer_post_id();
    if ( ! $offer_id ) return '';
    return esc_html( get_the_title( $offer_id ) );
});

add_shortcode( 'bm_oferta_opis', function() {
    $offer_id = bm_get_current_offer_post_id();
    if ( ! $offer_id ) return '';
    $post = get_post( $offer_id );
    if ( ! $post ) return '';
    return wp_kses_post( wpautop( (string) $post->post_content ) );
});

add_shortcode( 'bm_oferta_obrazek', function( $atts ) {
    $atts = shortcode_atts([
        'size'  => 'medium',
        'class' => '',
    ], $atts );
    $offer_id = bm_get_current_offer_post_id();
    if ( ! $offer_id || ! has_post_thumbnail( $offer_id ) ) return '';
    return get_the_post_thumbnail( $offer_id, $atts['size'], [ 'class' => $atts['class'] ] );
});

add_shortcode( 'bm_oferta_kategorie', function( $atts ) {
    $atts = shortcode_atts([ 'sep' => ', ' ], $atts );
    $offer_id = bm_get_current_offer_post_id();
    if ( ! $offer_id ) return '';
    $names = bm_get_offer_term_names( $offer_id, 'dostawca_kategoria', $atts['sep'] );
    return esc_html( $names );
});

add_shortcode( 'bm_oferta_termin', function() {
    $offer_id = bm_get_current_offer_post_id();
    if ( ! $offer_id ) return '';
    $names = bm_get_offer_term_names( $offer_id, 'oferta_termin', ', ' );
    return esc_html( $names );
});

add_shortcode( 'bm_oferta_budzet', function() {
    $offer_id = bm_get_current_offer_post_id();
    if ( ! $offer_id ) return '';
    $names = bm_get_offer_term_names( $offer_id, 'oferta_budzet', ', ' );
    return esc_html( $names );
});

add_shortcode( 'bm_oferta_tryb', function() {
    $offer_id = bm_get_current_offer_post_id();
    if ( ! $offer_id ) return '';
    $val = function_exists('get_field') ? get_field('oferta_tryb', $offer_id) : get_post_meta($offer_id, 'oferta_tryb', true);
    return esc_html( (string) $val );
});

add_shortcode( 'bm_oferta_obszar', function() {
    $offer_id = bm_get_current_offer_post_id();
    if ( ! $offer_id ) return '';
    $val = function_exists('get_field') ? get_field('oferta_obszar', $offer_id) : get_post_meta($offer_id, 'oferta_obszar', true);
    return esc_html( (string) $val );
});

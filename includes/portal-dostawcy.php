...
add_action('woocommerce_account_supplier-offers_endpoint', function(){
    ...
    $bm_render_offer_custom_fields = function($args = []) use ($offer_cat_limit) {
        $defaults = [
            'title'      => '',
            'cats'       => [],
            'term'       => 0,
        ...
        ob_start();
        echo '<div class="bm-field bm-field--offer-title"><label>Tytuł oferty</label><input type="text" name="bm_offer_title" value="'.esc_attr($args['title']).'" required maxlength="180"></div>';
        echo '<div class="bm-field bm-field--offer-cats"><label>Kategoria oferty (max '.(int)$offer_cat_limit.')</label>';
        bm_render_tax_tree('dostawca_kategoria','bm_offer_categories',(array)$args['cats'],$offer_cat_limit);
        echo '</div>';
        ...
        if (!is_wp_error($offer_term_options)){
            foreach($offer_term_options as $term_opt){
                echo '<option value="'.esc_attr((int)$term_opt->term_id).'" '.selected((int)$args['term'], (int)$term_opt->term_id, false).'>'.esc_html($term_opt->name).'</option>';
            }
        }
        echo '</select></div>';
        ...
        if (!is_wp_error($offer_budget_options)){
            foreach($offer_budget_options as $budget_opt){
                echo '<option value="'.esc_attr((int)$budget_opt->term_id).'" '.selected((int)$args['budget'], (int)$budget_opt->term_id, false).'>'.esc_html($budget_opt->name).'</option>';
            }
        }
        echo '</select></div>';

        echo '<div class="bm-field bm-field--offer-mode"><label>Tryb realizacji</label>';
        echo '<select name="bm_offer_tryb" class="bm-offer-tryb" data-target=".bm-field--offer-obszar">';
        echo '<option value="">— wybierz —</option>';
        echo '<option value="zdalnie" '.selected($args['tryb'], 'zdalnie', false).'>Zdalnie</option>';
        echo '<option value="lokalnie" '.selected($args['tryb'], 'lokalnie', false).'>Lokalnie / obszar</option>';
        echo '</select></div>';

        $obszar_style = ($args['tryb'] === 'lokalnie') ? '' : ' style="display:none"';
        echo '<div class="bm-field bm-field--offer-obszar"'.$obszar_style.'><label>Wprowadź miasto / obszar</label><input type="text" name="bm_offer_obszar" value="'.esc_attr($args['obszar']).'" maxlength="120"></div>';
    ...
    if ($mode === 'new'){
        ...
        $custom_fields_html = $bm_render_offer_custom_fields();
        ...
        acf_form([
            ...
            'label_placement'  => 'top',
            'html_before_fields'=> '<div class="bm-offer-form-acf">'.$custom_fields_html,
    ...
    if ($mode === 'edit' && $edit_id){
        ...
        $offer_tryb = function_exists('get_field') ? (string) get_field('oferta_tryb', $offer->ID) : (string) get_post_meta($offer->ID, 'oferta_tryb', true);
        $offer_obszar = function_exists('get_field') ? (string) get_field('oferta_obszar', $offer->ID) : (string) get_post_meta($offer->ID, 'oferta_obszar', true);
        ...
        $custom_fields_html = $bm_render_offer_custom_fields([
            ...
            'cats'   => $offer_selected_cats,
            'term'   => (int)$offer_selected_term[0],
            'budget' => (int)$offer_selected_budget[0],
            'tryb'   => $offer_tryb,
            'obszar' => $offer_obszar,
        ]);
    ...
    echo '<script>(function(){
      ...
      var tryb = offerWrap.querySelector(".bm-offer-tryb");
      if(tryb){
        var target = offerWrap.querySelector(tryb.getAttribute("data-target") || ".bm-field--offer-obszar");
        var syncTryb = function(){
          if(!target) return;
          target.style.display = (tryb.value === "lokalnie") ? "block" : "none";
        };
        tryb.addEventListener("change", syncTryb);
        syncTryb();
      }
...
add_filter('acf/prepare_field', function($field){
    ...
    if (strpos($uri, 'supplier-offers') === false) return $field;
    ...
    if (in_array((string)$field['name'], $hide_names, true)) {
        return false;
    }
    return $field;
}, 20);
...
add_action('acf/save_post', function($post_id){
    ...
    $offer_tryb = isset($_POST['bm_offer_tryb']) ? sanitize_text_field(wp_unslash($_POST['bm_offer_tryb'])) : '';
    $offer_obszar = isset($_POST['bm_offer_obszar']) ? sanitize_text_field(wp_unslash($_POST['bm_offer_obszar'])) : '';
    update_post_meta($post_id, 'oferta_tryb', $offer_tryb);
    update_post_meta($post_id, 'oferta_obszar', $offer_obszar);
    if (function_exists('update_field')) {
        @update_field('oferta_tryb', $offer_tryb, $post_id);
        @update_field('oferta_obszar', $offer_obszar, $post_id);
    }
}, 30);
